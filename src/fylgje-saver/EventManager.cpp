...
void bifrost::data::EventManager::save_to(hdf5::node::Group group) const {
  ...
  if (histogram_manager.has_value()) {
    histogram_manager->save_to(group);
  }
  if (store_pixels) {
    pixel_manager.save_to(group);
  }

  std::cout << "Saved data including " << messages.size() << " readouts to HDF5 file\n";
}

void bifrost::data::EventManager::save_to(hdf5::file::File file, std::optional<std::string> group) const {
  auto root = file.root();
...
void bifrost::data::EventManager::save_to(std::filesystem::path file, std::optional<std::string> group) const{
