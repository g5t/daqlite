...
void bifrost::data::PixelManager::save_to(hdf5::node::Group group) const {
  ...
  auto pds = group.create_dataset("pixels", datatype, dataspace, datasetCreationList);
  pds.attributes.create_from("wrap_order", pixel_order);
  pds.write(pixel_data);
}


void bifrost::data::PixelManager::save_to(hdf5::file::File file, std::optional<std::string> group) const {
  auto root = file.root();
...
void bifrost::data::PixelManager::save_to(std::filesystem::path file, std::optional<std::string> group) const{
